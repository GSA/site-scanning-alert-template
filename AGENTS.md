...
- `IssueClient._request` retries GETs on network errors (`retry_on_network_error=True`) but **never** retries a POST on a `URLError`/`TimeoutError` - the request may have already succeeded server-side, and a retried `POST /issues` risks filing a duplicate. HTTP *status* responses (5xx/429/secondary-rate-limit 403) are retried for both verbs, since a status proves the server rejected rather than maybe-applied the request. The `f"... HTTP {e.code} - {body}"` message shape is load-bearing: `ensure_label` sniffs `"HTTP 404"` out of it.
