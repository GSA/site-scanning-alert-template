...
class TestIssueClientMock(unittest.TestCase):
    ...
    def test_find_open_issue_paginates(self):
        ...
        page_2 = [{'number': 100, 'html_url': 'u100', 'body': '<!-- site-scanning-alert: alerts:abc123 -->\n\nbody'}]

        with patch.object(client, '_request', side_effect=[page_1, page_2]) as mock_request:
            result = client.find_open_issue(['site-scanning-alert'], '<!-- site-scanning-alert: alerts:abc123 -->')
    ...
    def test_find_open_issue_skips_null_body_without_crashing(self):
        ...
        issues = [
            ...
            {'number': 2, 'html_url': 'u2', 'body': '<!-- site-scanning-alert: alerts:abc123 -->\n\nbody'},
        ]
